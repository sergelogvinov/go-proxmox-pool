...
func newClusterEnv() []*pxpool.ClusterConfig {
...
func newClusterEnvWithFiles(tokenIDPath, tokenSecretPath string) []*pxpool.ClusterConfig {
	cfg := []*pxpool.ClusterConfig{
		{
			URL:             "https://127.0.0.1:8006/api2/json",
			Insecure:        false,
...
func TestNewClient(t *testing.T) {
	...
	pxClient, err := pxpool.NewProxmoxPool([]*pxpool.ClusterConfig{})
	assert.NotNil(t, err)
	assert.Nil(t, pxClient)
	assert.Equal(t, pxpool.ErrClustersNotFound, err)
...
func TestNewClientRejectsNegativeCacheTTL(t *testing.T) {
	...
	pxClient, err := pxpool.NewProxmoxPool(cfg, pxpool.WithCacheTTL(pxpool.ResourceKindVM, -1))
	assert.NotNil(t, err)
	assert.Nil(t, pxClient)
}
...
func TestList(t *testing.T) {
	...
	assert.ElementsMatch(t, []string{"cluster-1", "cluster-2"}, pxClient.List())
}
...
func TestGet(t *testing.T) {
	...
	pxapi, err := pxClient.Get("test")
	assert.NotNil(t, err)
	assert.Nil(t, pxapi)
	assert.Equal(t, pxpool.ErrClusterNotFound, err)

	pxapi, err = pxClient.Get("cluster-1")
	assert.Nil(t, err)
...
func TestClusterCheck(t *testing.T) {
	...
	err = pxClient.Cluster("missing").Check(t.Context())
	assert.NotNil(t, err)
	assert.Equal(t, pxpool.ErrClusterNotFound, err)

	err = pxClient.Cluster("cluster-1").Check(t.Context())
	assert.NotNil(t, err)
	assert.Contains(t, err.Error(), "cluster cluster-1")
}
